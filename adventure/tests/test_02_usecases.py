...
class MockJourneyRepository(repositories.JourneyRepository):
    ...
    def stop_journey(self, started_journey: models.Journey) -> models.Journey:
        started_journey.end = timezone.now().date()
        return started_journey
...
class TestStopJourney:
    def test_stop(self):
        ...
        usecase = usecases.StartJourney(repo, notifier).set_params(data)
        journey = usecase.execute()
        usecases.StopJourney(repo).set_params(journey).execute()
